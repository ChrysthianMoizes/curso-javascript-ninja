...
cpfs.forEach( function(cpf) {
	console.log(cleanCPF(cpf).replace(regex, function(cpf, prt1, prt2, prt3, prt4){
...
regex = /<(\w+)>([^<]+)<(\/\w+)>/g;
frase.replace(regex, function(frase, abertura, corpo, fechamento){
						console.log('<'+abertura+'>'+'O texto dentro da tag "'+abertura+'" é "'+corpo+'"<'+fechamento+'>');
					}
);
})();